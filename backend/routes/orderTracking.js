...
router.get('/:orderId/tracking', auth, async (req, res) => {
  try {
    const order = await Order.findById(req.params.orderId)
      ...
      .populate({
        path: 'deliveryPerson',
        select: 'name phone profilePicture rating totalRatings totalDeliveries currentLocation vehicleType vehicleNumber'
      });
    ...
    if (userId !== customerId && userId !== deliveryPersonId) {
...
// Auto-progression function (30 seconds per stage)
async function startAutoProgression(orderId) {
  const STAGE_DURATION = 30000; // 30 seconds
  const stages = ['To Restaurant', 'Preparing', 'Ready to Pickup', 'Rider En Route', 'Reached'];
