...
router.post('/create-payment-intent', auth, async (req, res) => {
  try {
    ...
    const { orderType, donationAmount, selectedNGO } = req.body;
    const donation = {};
    let restaurantAmount = total; // Restaurant gets the full order amount
    
    if (orderType === 'both' && donationAmount > 0 && selectedNGO) {
      donation.amount = donationAmount;
      donation.ngo = selectedNGO;
      donation.donatedAt = new Date();
      donation.transferredToNGO = false;
      
      // Restaurant gets full order amount, donation is additional
      // No need to subtract - donation is added on top of the order
      restaurantAmount = total; // Keep full amount for restaurant
      
      // Fetch NGO name from User model
      try {
        const User = require('../models/User');
        console.log('🔍 Fetching NGO with ID:', selectedNGO);
        const ngo = await User.findById(selectedNGO);
        console.log('🔍 Found NGO:', ngo ? { id: ngo._id, name: ngo.name, organizationName: ngo.organizationName, role: ngo.role } : 'NOT FOUND');
        
        if (ngo) {
          donation.ngoName = ngo.organizationName || ngo.name;
          console.log(`💝 Donation of ৳${donationAmount} to ${donation.ngoName}`);
        } else {
          console.error('❌ NGO not found with ID:', selectedNGO);
          donation.ngoName = 'NGO (Not Found)';
        }
      } catch (e) {
        console.error('❌ Error fetching NGO name:', e);
        donation.ngoName = 'NGO (Error)';
      }
    }
...
router.post('/confirm-payment', auth, async (req, res) => {
  try {
    ...
    // Don't set stageStartTime yet - wait for delivery person assignment
    ...
    try {
      ...
      if (assignmentResult.success) {
        ...
        // Start retry process with callback to start auto-progression when assigned
        retryAssignmentWithDelay(order._id, 1, 3, (orderId) => {
          setTimeout(() => {
            startOrderAutoProgression(orderId);
          }, 1000);
        });
