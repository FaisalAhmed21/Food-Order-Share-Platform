...
const OrderTrackingDetail = () => {
  ...
  useEffect(() => {
    ...
    // If no stageStartTime, delivery person not assigned yet - show waiting state
    if (!orderData.order.stageStartTime) {
      console.log('⏳ Waiting for delivery person assignment...');
    ...
    console.log(`⏱️ Stage: ${orderData.order.currentStage}, Elapsed: ${elapsed}s, Remaining: ${remaining}s`);
  ...
  }, [orderData?.order?.currentStage, orderData?.order?.stageStartTime]);
  ...
  if (order.status === 'cancelled') {
    return (
      <div className="tracking-container">
        ...
        <div className="cancelled-order-section">
          ...
          {order.deliveryAssignmentMessage && (
            <div className="assignment-message">
              <p>{order.deliveryAssignmentMessage}</p>
            </div>
          )}
          ...
          <div className="cancelled-details">
            ...
            <div className="info-card">
              <h4>🍽️ Restaurant: {restaurant.name}</h4>
            ...
            <div className="refund-info">
              ...
              <p>Your payment of <strong>৳{order.pricing.total}</strong> will be refunded within 5-7 business days.</p>
      ...
      <div className="stage-progress-section">
        <div className="timer-display">
          {!order.stageStartTime ? (
            <>
              <span className="timer-label">🔍 Finding delivery person...</span>
              <span className="timer-value waiting">Please wait</span>
            </>
          ) : (
            <>
          ...
          {order.donation && order.donation.amount > 0 && (
            <div className="donation-info-box">
              ...
              <div className="donation-content">
                ...
                <p>You donated <strong>৳{order.donation.amount.toFixed(2)}</strong> to <strong>{order.donation.ngoName || order.donation.ngo?.organizationName || order.donation.ngo?.name || 'NGO'}</strong></p>
                <p className="donation-note">Your generosity helps feed those in need.</p>
              </div>
            </div>
          )}
        </div>
