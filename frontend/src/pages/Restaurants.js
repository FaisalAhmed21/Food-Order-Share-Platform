...
const Restaurants = () => {
  ...
  const fetchRestaurants = async () => {
    try {
      ...
      // Pagination - reduced limit for faster loading
      params.append('page', 1);
      params.append('limit', 50);

      console.log('Fetching restaurants from:', `http://localhost:5000/api/restaurants/all?${params.toString()}`);

      // Add timeout to prevent hanging forever
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 10000); // 10 second timeout

      const response = await fetch(`http://localhost:5000/api/restaurants/all?${params.toString()}`, {
      ...
      if (error.name === 'AbortError') {
        setError('Request timeout. Please check your backend server.');
      } else {
  ...
  const fetchSuggestions = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/restaurants/search?q=${searchQuery}`);
      const data = await response.json();
