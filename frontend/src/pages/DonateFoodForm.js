...
const DonateFoodForm = () => {
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!selectedNGO) newErrors.selectedNGO = 'Please select an NGO to donate to';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add selected NGO
      if (selectedNGO) {
        formPayload.append('ngo', selectedNGO);
      }
      ...
      if (data.success) {
        const donationId = data.donationId || data.donation?._id;
        console.log('✅ Donation created with ID:', donationId);
        
        // Show success message
        alert('✅ Donation published successfully! Volunteer is being assigned...');
        
        // Navigate to donation tracking page to watch the 4-stage flow
        if (donationId) {
          navigate(`/donation-tracking/${donationId}`);
        } else {
          // Fallback to my-donations if no ID
      ...
      case 1:
        return (
          <div className="form-step">
            ...
            <div className="form-group">
              <label>Select NGO to Donate To *</label>
              {ngos.length === 0 ? (
                <p className="loading-ngos">⏳ Loading NGOs...</p>
              ) : (
                <select
                  value={selectedNGO}
                  onChange={(e) => setSelectedNGO(e.target.value)}
                  className={errors.selectedNGO ? 'error' : ''}
                  required
                >
              ...
              {errors.selectedNGO && <span className="error-text">{errors.selectedNGO}</span>}
            </div>
          </div>
        );
