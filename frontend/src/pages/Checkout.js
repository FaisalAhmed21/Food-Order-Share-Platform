...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    // Fetch NGOs for donation selection
    fetchNGOs();
    fetchAddresses();
    fetchUserContact();
  }, []);
  ...
  const fetchNGOs = async () => {
    try {
      console.log('🔍 Fetching NGOs from backend...');
      const response = await fetch('http://localhost:5000/api/ngo/all-ngos', {
        headers: {
      ...
      if (data.success) {
        console.log('✅ NGOs fetched successfully:', data.ngos.length, 'NGOs found');
        setNgos(data.ngos || []);
        // Set first NGO as default
        if (data.ngos && data.ngos.length > 0) {
          setSelectedNGO(data.ngos[0]._id);
          console.log('🎯 Default NGO selected:', data.ngos[0].ngoName || data.ngos[0].organizationName);
        } else {
  ...
  const processPayment = async () => {
    ...
    if (orderMode === 'both' && donateAmount > 50 && !otpRequired) {
      setOtpRequired(true);
      // Send OTP
    ...
    try {
      ...
      const paymentResponse = await fetch('http://localhost:5000/api/payments/create-payment-intent', {
        ...
        body: JSON.stringify({
          ...
          orderType: orderMode,
          donationAmount: orderMode === 'both' ? donateAmount : 0,
          selectedNGO: orderMode === 'both' ? selectedNGO : null
        })
      });
    ...
    <div className="checkout-container">
      ...
      <div className="checkout-content">
        ...
        <div className="checkout-form">
          ...
          {/* Order Mode - Hide donation options for NGO users */}
          {userRole !== 'NGO' && (
            <div className="checkout-section">
              <h2>🍽️ Order Mode</h2>
            ...
            {orderMode === 'both' && (
              <div className="donation-details">
                ...
                <div className="ngo-selection">
                  <label>Select NGO to receive donation:</label>
                  {ngos.length === 0 ? (
                    <div className="ngo-loading">
                      <p>⏳ Loading NGOs...</p>
                      <small>If no NGOs appear, please check the console for errors</small>
                    </div>
                  ) : (
                    <select 
                      value={selectedNGO} 
                      ...
                      required
                    >
                      <option value="">Select an NGO ({ngos.length} available)</option>
                      {ngos.map(ngo => (
                        <option key={ngo._id} value={ngo._id}>
                          {ngo.ngoName || ngo.organizationName || 'Unnamed NGO'} {ngo.isVerified && '✓'}
                        </option>
                      ))}
                    </select>
                  )}
